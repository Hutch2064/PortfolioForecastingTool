...
def plot_forecasts(port_rets, start_cap, central, paths):
    ...
    # ---- find best/worst 1-year rolling segments ----
    log_rets = np.log(central[1:] / central[:-1])
    roll_sum = pd.Series(log_rets).rolling(252).sum()
    best_idx = roll_sum.idxmax()
    worst_idx = roll_sum.idxmin()
    ...
    ax.plot(dates, port_cum.iloc[-1] * central / central[0], color="blue", lw=2, label="Forecast")
    ...
    ax2.plot(dates, start_cap * central / central[0], color="blue", lw=2, label="Forecast")
    if best_start is not None:
