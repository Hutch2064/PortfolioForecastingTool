...
# ---------- Median-Ending Subset "Modal-Like" Path ----------
def find_median_ending_modal_like_path(paths: np.ndarray):
    endings = paths[:, -1]
    ...
    norms = np.linalg.norm(subset - subset.mean(axis=1, keepdims=True), axis=1)
    centered_subset = subset - subset.mean(axis=1, keepdims=True)
    centered_median = median_series - median_series.mean()
    corr = (centered_subset @ centered_median) / (norms * np.linalg.norm(centered_median))
    return subset[np.argmax(corr)]
    ...
    if st.button("Run Forecast"):
        try:
            ...
            for i, seed in enumerate(range(ENSEMBLE_SEEDS)):
                ...
                seed_medoids.append(find_median_ending_modal_like_path(sims))
                progress = (i+1)/ENSEMBLE_SEEDS
                progress_bar.progress(progress)
                status_text.text(f"Running forecasts... {i+1}/{ENSEMBLE_SEEDS}")
            progress_bar.empty()
            final_medoid = find_median_ending_modal_like_path(np.vstack(seed_medoids))
