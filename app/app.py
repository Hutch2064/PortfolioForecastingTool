...
def fetch_prices_monthly(tickers, start=DEFAULT_START):
    ...
    if isinstance(data.columns, pd.MultiIndex):
        ...
        else: raise ValueError("Missing Close field.")
    else:
...
def portfolio_returns_monthly(prices, weights, rebalance):
    ...
    for i, date in enumerate(rets.index):
        if i > 0: port_val *= (1 + (rets.iloc[i] @ cw))
        port_vals.append(port_val)
        if date in rebalance_dates: cw = weights.copy()
...
def _eval_params_on_split(params, train_X, train_Y, test_X, test_Y):
    m = LGBMRegressor(**{k:v for k,v in params.items() if k!="df"})
...
def tune_across_recent_oos_years(X, Y, years_back=5, seed=GLOBAL_SEED, n_trials=50):
    ...
    if years:
        last_rmse,last_da=_eval_params_on_split(consensus,
            X.loc[:f"{years[-1]-1}-12-31"],Y.loc[:f"{years[-1]-1}-12-31"],
            X.loc[f"{years[-1]}-01-01":f"{years[-1]}-12-31"],Y.loc[f"{years[-1]}-01-01":f"{years[-1]}-12-31"])
    return consensus, details, last_rmse, last_da

# ---------- Modal-Ending Medoid ----------
def find_modal_ending_medoid(paths):
    ends = paths[:, -1]
    counts,bins = np.histogram(ends,bins=100)
    mode_idx = np.argmax(counts)
    lo,hi = bins[mode_idx], bins[mode_idx+1]
    subset_idx = np.where((ends>=lo)&(ends<=hi))[0]
    if len(subset_idx)==0:
        subset_idx=np.argsort(np.abs(ends-np.median(ends)))[:max(1,len(paths)//20)]
    subset=paths[subset_idx]
    med=np.median(subset,axis=0)
    dists=np.sum(np.abs(subset-med),axis=1)
    return subset[np.argmin(dists)]
...
# ---------- Main ----------
def main():
    st.title("Portfolio Forecasting Toolio2")
    ...
    if st.button("Run Forecast"):
        try:
            ...
            for i in range(ENSEMBLE_SEEDS):
                ...
                seed_meds.append(find_modal_ending_medoid(sims))
                progress_bar.progress((i+1)/ENSEMBLE_SEEDS)
                status_text.text(f"Running forecasts... {i+1}/{ENSEMBLE_SEEDS}")
            progress_bar.empty()

            final_med=find_modal_ending_medoid(np.vstack(seed_meds))
            stats=compute_forecast_stats_from_path(final_med,start_cap,port_rets.index[-1])
            ...
            st.metric("Forecasted Portfolio Value",f"${float(final_med[-1])*start_cap:,.2f}")
            plot_forecasts(port_rets,start_cap,final_med,rebalance_label)
