...
def plot_forecasts(port_rets, start_cap, central, paths):
    ...
    # === Compute best/worst 1-year periods ===
    window = 252
    log_central = np.log(central)
    rolling = log_central[window:] - log_central[:-window]
    best_idx = np.argmax(rolling)
    worst_idx = np.argmin(rolling)
    best_ret = np.exp(rolling[best_idx]) - 1
    worst_ret = np.exp(rolling[worst_idx]) - 1
    ...
    # Blue line up to optimal horizon
    ...
    # Lighter blue after optimal horizon
    ax.plot(dates[opt_days:], port_cum.iloc[-1] * central[opt_days:] / central[0],
            color="#6fa8dc", lw=2, label=f"Beyond Optimal Horizon")

    # Highlight best/worst 1-year segments
    ax.plot(dates[best_idx:best_idx+window], port_cum.iloc[-1] * central[best_idx:best_idx+window] / central[0],
            color="green", lw=3, label=f"Best 1yr ~ {best_ret*100:.1f}%")
    ax.plot(dates[worst_idx:worst_idx+window], port_cum.iloc[-1] * central[worst_idx:worst_idx+window] / central[0],
            color="red", lw=3, label=f"Worst 1yr ~ {worst_ret*100:.1f}%")
    ...
    ax2.plot(dates[best_idx:best_idx+window], start_cap * central[best_idx:best_idx+window] / central[0],
             color="green", lw=3, label=f"Best 1yr ~ {best_ret*100:.1f}%")
    ax2.plot(dates[worst_idx:worst_idx+window], start_cap * central[worst_idx:worst_idx+window] / central[0],
             color="red", lw=3, label=f"Worst 1yr ~ {worst_ret*100:.1f}%")
