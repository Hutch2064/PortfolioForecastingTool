...
def fetch_macro_features(start=DEFAULT_START):
    tickers = ["^VIX", "^MOVE", "^TNX", "^IRX", "GC=F", "DX-Y.NYB", "^VVIX", "^SKEW"]
...
def tune_across_recent_oos_years(X, Y, years_back=5, seed=GLOBAL_SEED, n_trials=100):
    ...
    for y in years:
        ...
        params_all.append(study.best_trial.params)
    return _median_params(params_all), None, None, None
...
def block_bootstrap_residuals(residuals, size, block_len, rng):
    ...
    starts = rng.choice(np.arange(0, n - block_len + 1), int(np.ceil(size / block_len)), replace=True)
...
def run_monte_carlo_paths(model, X_base, Y_base, residuals, sims_per_seed, rng, seed_id=None, block_len=3, indicator_models=None):
    ...
    for t in range(horizon):
        ...
        vol_3m = inc.iloc[:, -3:].std(axis=1).values if t >= 2 else np.zeros(sims_per_seed)
        vol_6m = inc.iloc[:, -6:].std(axis=1).values if t >= 5 else np.zeros(sims_per_seed)
        vol_12m = inc.iloc[:, -12:].std(axis=1).values if t >= 11 else np.zeros(sims_per_seed)
        ...
        if indicator_models:
            for feat in ["VIX","MOVE","YC_Spread","Gold","DXY","SKEW"]:
                if feat in indicator_models:
                    idx = list(X_base.columns).index(feat)
...
def compute_medoid_path(paths):
    ...
    sims = (normed @ mean_traj) / (np.linalg.norm(normed, axis=1)*np.linalg.norm(mean_traj)+1e-9)
...
def plot_feature_attributions(model, X, medoid_states):
    ...
    shap_fore_all = [np.abs(expl.shap_values(pd.DataFrame([s], columns=X.columns))).reshape(-1) for s in medoid_states]
    shap_fore = np.mean(shap_fore_all, axis=0)
    feats = X.columns
    pos = np.arange(len(feats))
    fig, ax = plt.subplots(figsize=(10,6))
    ax.bar(pos-0.2, shap_hist/shap_hist.sum()*100, width=0.4, label="Backtest")
    ax.bar(pos+0.2, shap_fore/shap_fore.sum()*100, width=0.4, label="Forecast")
...
def plot_forecasts(port_rets, start_cap, central, reb_label):
    ...
    ax.plot([last,*dates], [port_cum.iloc[-1], *fore], label="Forecast", lw=2)
...
def main():
    ...
    if st.button("Run Forecast"):
        try:
            ...
            evolve_feats = ["VIX","MOVE","YC_Spread","Gold","DXY","SKEW"]
            indicators = train_indicator_models(X, evolve_feats)

            all_paths = []
            for i in range(ENSEMBLE_SEEDS):
                rng = np.random.default_rng(GLOBAL_SEED+i)
                sims = run_monte_carlo_paths(model,X,Y,res,SIMS_PER_SEED,rng,i,3,indicators)
